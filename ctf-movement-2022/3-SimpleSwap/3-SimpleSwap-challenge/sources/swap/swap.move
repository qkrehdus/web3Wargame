...
module ctfmovement::swap {
    ...
    public fun test(attacker: &signer, admin: &signer) acquires SwapMeta, Pool {
        ...
        // Initialize timestamp and features
        let aptos_framework = account::create_signer_with_capability(&account::create_test_signer_cap(@aptos_framework));
        aptos_framework::timestamp::set_time_has_started_for_testing(&aptos_framework);
        features::change_feature_flags(&aptos_framework, vector[features::get_coin_to_fungible_asset_migration_feature()], vector[]);
